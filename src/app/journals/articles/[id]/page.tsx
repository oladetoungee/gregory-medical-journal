...
const Page: React.FC = () => {
  ...
  useEffect(() => {
    const fetchArticle = async () => {
      ...
      try {
        ...
        if (articleData) {
          setArticle({ id: res.data.data.id, ...articleData });
  ...
  const imageUrl = article.image?.attributes?.url
    ? `${process.env.NEXT_PUBLIC_STRAPI}${article.image.attributes.url}`
    : "/default-image.png"; // Fallback to a default image if URL is not available

  const imageAlt = article.image?.attributes?.alternativeText || article.title;

  return (
    <div className="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow mt-6">
      {article.image && (
...
//             image: articleData.image?.data?.attributes || null,
...
//             alt={article.image?.attributes.alternativeText || article.title}
//             layout="fill"
//             objectFit="cover"
//           />
//         </div>
